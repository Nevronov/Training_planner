...
def insert(con, table, values): # запрос на создание строки для таблицы
    ...
    if table == 'users':
        ...
        a.execute("INSERT INTO Workouts(w_user, w_name, w_description) VALUES (?,?,?)", values)
    elif table == 'conducting_workouts':
        a.execute("INSERT INTO Conducting_Workouts(wc_workout, wc_date, wc_status) VALUES (?,?,?)", values)
    elif table == 'muscle_groups':
        a.execute("INSERT INTO Muscle_groups(m_name) VALUES (?)", values)
    elif table == 'exercises':
        a.execute("INSERT INTO Exercises(e_name, e_description, e_user) VALUES (?,?,?)", values)
    ...
    elif table == 'muscles_list':
        a.execute("INSERT INTO Muscles_list(ml_muscle, ml_exercises) VALUES (?,?)", values)
    con.commit()
...
def delete_all_db(con):
    ...
    cursor.executescript(f"""DELETE FROM Exercises_list;
                             ...
                             DELETE FROM Conducting_Workouts;
